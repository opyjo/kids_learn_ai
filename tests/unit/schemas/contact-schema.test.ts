...
describe('Contact Form Schema', () => {
  ...
  describe('name field validation', () => {
    ...
    it('should pass with name at exactly 100 characters', () => {
      ...
      const result = contactFormSchema.safeParse({ ...validData, name: maxName })
      expect(result.success).toBe(true)
    })
  })
  ...
  describe('subject field validation', () => {
    ...
    it('should fail when subject exceeds 200 characters', () => {
      ...
      const result = contactFormSchema.safeParse({ ...validData, subject: longSubject })
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Subject must not exceed 200 characters')
      }
    })
  })
  ...
  describe('message field validation', () => {
    ...
    it('should fail when message is too short', () => {
      const result = contactFormSchema.safeParse({ ...validData, message: 'Hi there' })
      expect(result.success).toBe(false)
      if (!result.success) {
    ...
    it('should fail when message exceeds 1000 characters', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Message must not exceed 1000 characters')
      }
    })

    it('should pass with message at exactly 1000 characters', () => {
      const maxMessage = 'A'.repeat(1000)
      const result = contactFormSchema.safeParse({ ...validData, message: maxMessage })
      expect(result.success).toBe(true)
    })
  })
